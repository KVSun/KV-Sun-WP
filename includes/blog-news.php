<?php
	...
	begin_file(__FILE__);
?>
	<div id="news" class="blog-lists-blog clearfix">
	...
	?>
	 ...
	 if ( !empty($image) ) { ?>
					  ...
					  <?php } else { ?>
							<a itemprop="url" href="<?php  the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"><img  itemprop="image" style ="background-image :url(<?php echo get_template_directory_uri() . DEFAULT_IMG; ?>);background-repeat:no-repeat; background-size:contain;   background-position: 57% center; height: 172px;" alt="<?php //the_title(); ?>" /></a>
						<?php } ?>
	 </div>
		 <p>Photo By  <?php
